...
--throw a promotional Music Festival in the city
--we made the most money. Write a query that returns 
--one city that has the highest sum of invoice totals. Return both the city name & sum of all invoice

SELECT SUM(total)AS invoice_total,billing_city FROM invoice
GROUP BY billing_city
ORDER BY invoice_total DESC;
--Who is the best customer? The customer who has 
...
ORDER BY email;
--2. Let's invite the artists who have written the 
--most rock music in our dataset. Write a query 
--that returns the Artist name and total track
...
WHERE milliseconds>(
SELECT AVG(milliseconds) AS length_of_track FROM track)
ORDER BY milliseconds DESC;

--Question Set 3 – Advance
--Find how much amount spent by each customer on 
...
--most popular genre as the genre with the highest
--amount of purchases. Write a query that returns
--each country along with the top Genre. For 
--countries where the maximum number of purchases
